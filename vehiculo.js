...
app.get("/CarroPorId", (request, response) => {
 usuarioModelo.find(
...
app.get("/ConsultarCarros", (reqest, response) => {
 usuarioModelo.find(function (error, documentos) {
 response.send(documentos);
 });
});
 
app.post("/Agregarcarro", function (request, response) {
...
app.delete("/EliminarCarro", function (request, response) {
...
app.put("/EditarCarro", function (request, response) {
 const filter = { documento: request.body.documento };
 const update = { propietario: String(request.body.edad),
